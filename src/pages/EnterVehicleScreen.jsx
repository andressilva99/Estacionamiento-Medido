...
const EnterVehicleScreen = ({ navigation, route }) => {
    ...
    const [isOpenAlertNoticeNoGoBack, setIsOpenAlertNoticeNoGoBack] =
        useState(false);
    ...
    const [messageAlertNoticeNoGoBack, setMessageAlertNoticeNoGoBack] =
        useState();
        ...
        await constants.AXIOS_INST.get(`vehiculo/${data.patent}`, config)
            .then(async (response) => {
                ...
                if (msj.existe) {
                    ...
                    setMessageAlertNoticeNoGoBack(
                        `No se encontró ningún vehículo registrado con patente ${data.patent}. Por favor registre los datos de su vehículo.`
                    );
                    setIsOpenAlertNoticeNoGoBack(true);
                }
            })
            ...
            >
                    ...
                    {find ? (
                        ...
                        <Button
                            ...
                            onPress={handleSubmit(
                                async (data) => await findPatent(data)
                            )}
                        >
                            <HStack space={"lg"}>
                                <Text style={styles.textButton}>
                ...
                {find ? (
                    <VStack space={"sm"} width={"85%"}>
                        <Skeleton style={styles.skeletonDataVehicle} startColor={"#D1D6D9"}></Skeleton>
                        <Skeleton style={styles.skeletonDataVehicle} startColor={"#D1D6D9"}></Skeleton>
                        <Skeleton style={styles.skeletonDataVehicle} startColor={"#D1D6D9"}></Skeleton>
                        <Skeleton style={styles.skeletonDataVehicle} startColor={"#51be85"}></Skeleton>
                    </VStack>
                ) : vehicleFind ? (
                    <VStack space={"sm"}>
